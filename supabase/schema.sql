...
begin
  ...
  select
    enrollment.user_id,
    'discipleship_assignment_due'::public.notification_type,
    'Assignment due soon',
    coalesce(lesson.title, 'Assignment')
      || ' is due '
      || to_char(lesson.due_at, 'DD Mon YYYY HH24:MI'),
    'discipleship_classes',
    lesson.class_id
  from public.discipleship_lessons lesson
  join public.discipleship_enrollments enrollment
    on enrollment.class_id = lesson.class_id
  where lesson.lesson_type in ('assignment', 'project')
    and lesson.due_at is not null
    and lesson.due_at > timezone('utc', now())
    and lesson.due_at <= timezone('utc', now()) + interval '48 hours'
    and enrollment.status = 'approved'
    and enrollment.user_id = auth.uid()
    and not exists (
      select 1
      from public.discipleship_assignment_submissions submission
      where submission.lesson_id = lesson.id
        and submission.user_id = enrollment.user_id
        and submission.status in ('submitted', 'under_review', 'reviewed', 'completed')
    )
    and not exists (
      select 1
      from public.notifications notification
      where notification.user_id = auth.uid()
        and notification.notification_type = 'discipleship_assignment_due'
        and notification.entity_table = 'discipleship_classes'
        and notification.entity_id = lesson.class_id
        and notification.body like '%' || lesson.title || '%'
        and notification.created_at > timezone('utc', now()) - interval '52 hours'
    );
